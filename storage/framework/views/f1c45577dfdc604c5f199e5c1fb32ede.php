...
<div style="max-width: 600px; margin: 2rem auto; animation: fadeUp 0.6s ease-out;">
    ...
    <div style="background-color: var(--bg-card); border: 1px solid var(--accent); padding: 2.5rem; border-top: 4px solid var(--accent); position: relative; overflow: hidden;">
        ...
        <div style="text-align: center; margin-bottom: 2.5rem; position: relative;">
            ...
            <div style="font-size: 3rem; color: var(--text-main); font-weight: 900; font-family: monospace;">RM <?php echo e(number_format($total, 2)); ?></div>
        </div>

        <form method="POST" action="<?php echo e(route('checkout')); ?>" onsubmit="this.querySelector('button[type=submit]').disabled = true; this.querySelector('button[type=submit]').innerHTML = 'Processing Transaction...';">
            <?php echo csrf_field(); ?>
            ...
            <input type="text" class="form-control" placeholder="JOHN DOE" value="<?php echo e(Auth::user()->name); ?>" required style="font-family: inherit; font-weight: 500;">
            ...
            <a href="<?php echo e(route('cart.index')); ?>" class="btn w-100" style="margin-top: 1rem; text-align: center; color: var(--text-muted); font-size: 0.85rem; text-decoration: underline;">Return to Queue</a>
        </form>
    </div>
</div>
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/resources/views/checkout.blade.php ENDPATH**/ ?>